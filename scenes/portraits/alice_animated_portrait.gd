...
## Animated portrait for Alice with talking animation
...
func _ready() -> void:
	print("🔥 Alice animated portrait _ready() called! 🔥")
	...
	if not Engine.is_editor_hint():
		...
		print("Alice portrait: Signals connected")


## Called when text starts showing (character is speaking)
func _on_text_started(_text_info: Dictionary) -> void:
	print("Alice portrait: text_started signal received")
	_check_if_speaking()


## Called when text finishes
func _on_text_finished(_text_info: Dictionary) -> void:
	print("Alice portrait: text_finished signal received")
	stop_talking()


## Called when the speaker changes
func _on_speaker_updated(_character: DialogicCharacter) -> void:
	print("Alice portrait: speaker_updated signal received, character: ", _character)
	_check_if_speaking()


## Check if this portrait's character (Alice) is currently speaking
func _check_if_speaking() -> void:
	print("Alice portrait: Checking if speaking. character = ", character)
	if character:
		print("Alice portrait: character.display_name = ", character.display_name)
		var current_speaker = Dialogic.Text.get_current_speaker()
		print("Alice portrait: current_speaker = ", current_speaker)

		if character.display_name == "Alice":
			if current_speaker and current_speaker.display_name == "Alice":
				print("Alice portrait: Alice is speaking!")
				start_talking()
			else:
				print("Alice portrait: Alice is NOT speaking")
				stop_talking()
	else:
		print("Alice portrait: character is null!")
...
func start_talking() -> void:
	if animated_sprite and not is_talking:
		...
		print("Alice: Started talking animation")
...
func stop_talking() -> void:
	if animated_sprite and is_talking:
		is_talking = false
		animated_sprite.play("idle")
		print("Alice: Stopped talking, back to idle")
