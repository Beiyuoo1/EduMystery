...
## Animated portrait for Ben with talking animation
## Automatically plays "talking" animation during dialogue
## Returns to "idle" (Ben_half.png) when dialogue stops
...
func _ready() -> void:
	...
	if not Engine.is_editor_hint():
		...
		print("Ben portrait: Signals connected")


## Called when text starts showing (character is speaking)
func _on_text_started(_text_info: Dictionary) -> void:
	print("Ben portrait: text_started signal received")
	_check_if_speaking()


## Called when text finishes
func _on_text_finished(_text_info: Dictionary) -> void:
	print("Ben portrait: text_finished signal received")
	stop_talking()


## Called when the speaker changes
func _on_speaker_updated(_character: DialogicCharacter) -> void:
	print("Ben portrait: speaker_updated signal received, character: ", _character)
	_check_if_speaking()


## Check if this portrait's character (Ben) is currently speaking
func _check_if_speaking() -> void:
	print("Ben portrait: Checking if speaking. character = ", character)
	if character:
		print("Ben portrait: character.display_name = ", character.display_name)
		var current_speaker = Dialogic.Text.get_current_speaker()
		print("Ben portrait: current_speaker = ", current_speaker)

		if character.display_name == "Ben":
			if current_speaker and current_speaker.display_name == "Ben":
				print("Ben portrait: Ben is speaking!")
				start_talking()
			else:
				print("Ben portrait: Ben is NOT speaking")
				stop_talking()
	else:
		print("Ben portrait: character is null!")
...
func start_talking() -> void:
	if animated_sprite and not is_talking:
		...
		print("Ben: Started talking animation")
...
func stop_talking() -> void:
	if animated_sprite and is_talking:
		is_talking = false
		animated_sprite.play("idle")
		print("Ben: Stopped talking, back to idle")
