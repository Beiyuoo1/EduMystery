...
## Animated portrait for Celestine with talking animation
## Automatically plays "talking" animation during dialogue
## Returns to "idle" (Thebe_half.png) when dialogue stops
...
func _ready() -> void:
	...
	if not Engine.is_editor_hint():
		...
		print("Celestine portrait: Signals connected")


## Called when text starts showing (character is speaking)
func _on_text_started(_text_info: Dictionary) -> void:
	print("Celestine portrait: text_started signal received")
	_check_if_speaking()


## Called when text finishes
func _on_text_finished(_text_info: Dictionary) -> void:
	print("Celestine portrait: text_finished signal received")
	stop_talking()


## Called when the speaker changes
func _on_speaker_updated(_character: DialogicCharacter) -> void:
	print("Celestine portrait: speaker_updated signal received, character: ", _character)
	_check_if_speaking()


## Check if this portrait's character (Celestine) is currently speaking
func _check_if_speaking() -> void:
	print("Celestine portrait: Checking if speaking. character = ", character)
	if character:
		print("Celestine portrait: character.display_name = ", character.display_name)
		var current_speaker = Dialogic.Text.get_current_speaker()
		print("Celestine portrait: current_speaker = ", current_speaker)

		if character.display_name == "Celestine":
			if current_speaker and current_speaker.display_name == "Celestine":
				print("Celestine portrait: Celestine is speaking!")
				start_talking()
			else:
				print("Celestine portrait: Celestine is NOT speaking")
				stop_talking()
	else:
		print("Celestine portrait: character is null!")
...
func start_talking() -> void:
	if animated_sprite and not is_talking:
		...
		print("Celestine: Started talking animation")
...
func stop_talking() -> void:
	if animated_sprite and is_talking:
		is_talking = false
		animated_sprite.play("idle")
		print("Celestine: Stopped talking, back to idle")
