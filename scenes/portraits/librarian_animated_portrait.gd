...
## Animated portrait for Librarian with talking animation
## Automatically plays "talking" animation during dialogue
## Returns to "idle" (Librarian_half.png) when dialogue stops
...
func _ready() -> void:
	...
	if not Engine.is_editor_hint():
		...
		print("Librarian portrait: Signals connected")
...
func _check_if_speaking() -> void:
	if character:
		...
		if character.display_name == "Librarian":
			if current_speaker and current_speaker.display_name == "Librarian":
				start_talking()
			else:
				stop_talking()
