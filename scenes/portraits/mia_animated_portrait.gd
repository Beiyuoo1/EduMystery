...
## Animated portrait for Mia with talking animation
...
func _ready() -> void:
	print("🔥 Mia animated portrait _ready() called! 🔥")
	...
	if not Engine.is_editor_hint():
		...
		print("Mia portrait: Signals connected")


## Called when text starts showing (character is speaking)
func _on_text_started(_text_info: Dictionary) -> void:
	print("Mia portrait: text_started signal received")
	_check_if_speaking()


## Called when text finishes
func _on_text_finished(_text_info: Dictionary) -> void:
	print("Mia portrait: text_finished signal received")
	stop_talking()


## Called when the speaker changes
func _on_speaker_updated(_character: DialogicCharacter) -> void:
	print("Mia portrait: speaker_updated signal received, character: ", _character)
	_check_if_speaking()


## Check if this portrait's character (Mia) is currently speaking
func _check_if_speaking() -> void:
	print("Mia portrait: Checking if speaking. character = ", character)
	if character:
		print("Mia portrait: character.display_name = ", character.display_name)
		var current_speaker = Dialogic.Text.get_current_speaker()
		print("Mia portrait: current_speaker = ", current_speaker)

		if character.display_name == "Mia":
			if current_speaker and current_speaker.display_name == "Mia":
				print("Mia portrait: Mia is speaking!")
				start_talking()
			else:
				print("Mia portrait: Mia is NOT speaking")
				stop_talking()
	else:
		print("Mia portrait: character is null!")
...
func start_talking() -> void:
	if animated_sprite and not is_talking:
		...
		print("Mia: Started talking animation")
...
func stop_talking() -> void:
	if animated_sprite and is_talking:
		is_talking = false
		animated_sprite.play("idle")
		print("Mia: Stopped talking, back to idle")
