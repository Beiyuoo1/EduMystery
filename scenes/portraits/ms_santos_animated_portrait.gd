...
## Animated portrait for Ms. Santos with talking animation
## Automatically plays "talking" animation during dialogue
## Returns to "idle" (MsSantos_half.png) when dialogue stops
...
func _ready() -> void:
	...
	if not Engine.is_editor_hint():
		...
		print("Ms. Santos portrait: Signals connected")


## Called when text starts showing (character is speaking)
func _on_text_started(_text_info: Dictionary) -> void:
	print("Ms. Santos portrait: text_started signal received")
	_check_if_speaking()


## Called when text finishes
func _on_text_finished(_text_info: Dictionary) -> void:
	print("Ms. Santos portrait: text_finished signal received")
	stop_talking()


## Called when the speaker changes
func _on_speaker_updated(_character: DialogicCharacter) -> void:
	print("Ms. Santos portrait: speaker_updated signal received, character: ", _character)
	_check_if_speaking()


## Check if this portrait's character (Ms. Santos) is currently speaking
func _check_if_speaking() -> void:
	print("Ms. Santos portrait: Checking if speaking. character = ", character)
	if character:
		print("Ms. Santos portrait: character.display_name = ", character.display_name)
		var current_speaker = Dialogic.Text.get_current_speaker()
		print("Ms. Santos portrait: current_speaker = ", current_speaker)

		if character.display_name == "Ms. Santos":
			if current_speaker and current_speaker.display_name == "Ms. Santos":
				print("Ms. Santos portrait: Ms. Santos is speaking!")
				start_talking()
			else:
				print("Ms. Santos portrait: Ms. Santos is NOT speaking")
				stop_talking()
	else:
		print("Ms. Santos portrait: character is null!")
...
func start_talking() -> void:
	if animated_sprite and not is_talking:
		...
		print("Ms. Santos: Started talking animation")
...
func stop_talking() -> void:
	if animated_sprite and is_talking:
		is_talking = false
		animated_sprite.play("idle")
		print("Ms. Santos: Stopped talking, back to idle")
