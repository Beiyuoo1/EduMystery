...
## Animated portrait for Ms. Reyes with talking animation
## Automatically plays "talking" animation during dialogue
## Returns to "idle" when dialogue stops
...
func _ready() -> void:
	print("🔥 Ms. Reyes animated portrait _ready() called! 🔥")
	...
	if not Engine.is_editor_hint():
		...
		print("Ms. Reyes portrait: Signals connected")


## Called when text starts showing (character is speaking)
func _on_text_started(_text_info: Dictionary) -> void:
	print("Ms. Reyes portrait: text_started signal received")
	_check_if_speaking()


## Called when text finishes
func _on_text_finished(_text_info: Dictionary) -> void:
	print("Ms. Reyes portrait: text_finished signal received")
	stop_talking()


## Called when the speaker changes
func _on_speaker_updated(_character: DialogicCharacter) -> void:
	print("Ms. Reyes portrait: speaker_updated signal received, character: ", _character)
	_check_if_speaking()


## Check if this portrait's character (Ms. Reyes) is currently speaking
func _check_if_speaking() -> void:
	print("Ms. Reyes portrait: Checking if speaking. character = ", character)
	if character:
		print("Ms. Reyes portrait: character.display_name = ", character.display_name)
		var current_speaker = Dialogic.Text.get_current_speaker()
		print("Ms. Reyes portrait: current_speaker = ", current_speaker)

		if character.display_name == "Ms. Reyes":
			if current_speaker and current_speaker.display_name == "Ms. Reyes":
				print("Ms. Reyes portrait: Ms. Reyes is speaking!")
				start_talking()
			else:
				print("Ms. Reyes portrait: Ms. Reyes is NOT speaking")
				stop_talking()
	else:
		print("Ms. Reyes portrait: character is null!")
...
func start_talking() -> void:
	if animated_sprite and not is_talking:
		...
		print("Ms. Reyes: Started talking animation")
...
func stop_talking() -> void:
	if animated_sprite and is_talking:
		is_talking = false
		animated_sprite.play("idle")
		print("Ms. Reyes: Stopped talking, back to idle")
