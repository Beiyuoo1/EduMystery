...
## Animated portrait for Ryan (Cry) with talking animation
## Automatically plays "talking" animation during dialogue
## Returns to "idle" (frame 1 of cry animation) when dialogue stops
...
func _ready() -> void:
	...
	if not Engine.is_editor_hint():
		...
		print("Ryan (Cry) portrait: Signals connected")
