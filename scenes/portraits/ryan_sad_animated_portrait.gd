...
## Animated portrait for Ryan (Sad) with talking animation
## Automatically plays "talking" animation during dialogue
## Returns to "idle" (frame 1 of sad animation) when dialogue stops
...
func _ready() -> void:
	...
	if not Engine.is_editor_hint():
		...
		print("Ryan (Sad) portrait: Signals connected")
